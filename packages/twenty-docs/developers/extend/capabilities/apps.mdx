...
- `yarn twenty app:dev` will auto-generate two typed API clients in `node_modules/twenty-client-sdk`: `CoreApiClient` (for workspace data via `/graphql`) and `MetadataApiClient` (for workspace configuration and file uploads via `/metadata`).
...
Two typed clients are auto-generated by `yarn twenty app:dev` and stored in `node_modules/twenty-client-sdk` based on your workspace schema:
